...
def objective(logits, rows, answer_ids, weights, k):
    ...
    if weights.get('format_weight', 0.0) > 0:
        log_Z = torch.logsumexp(logp[k:2*k, answer_ids[:3]], dim=-1)
        format_loss = -log_Z
        loss = loss + weights['format_weight'] * .5 * (format_loss * ~exact_mask).mean()
    elif weights.get('semantic_weight', 0.0) > 0:
        log_q = logits[k:2*k, answer_ids[:3]].log_softmax(-1)
        mask = torch.tensor([[label in row for label in LABELS[:3]] for row in nli_allowed], device=logits.device)
        semantic_loss = -torch.logsumexp(log_q.masked_fill(~mask, -torch.inf), dim=-1)
        loss = loss + weights['semantic_weight'] * .5 * (semantic_loss * ~exact_mask).mean()
    elif weights.get('partial_weight', 0.0) > 0:
        loss = loss + weights['partial_weight'] * .5 * partial_nli
    loss = loss + weights['equivariance_weight'] * equivariance
